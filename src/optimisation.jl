...
function surrogatefit(model, t, y_exp, guess, lb, ub, x_tol, f_tol, f_calls, initsamp=50)
    ...
    sample = [scale(collect(corner)) for corner in corners] # TODO: convert to vector of tuples
    # Fill the remaining sample points using sample(...)
    if length(sample) < initsamp
    ...
    push!(sample, tuple(scale(guess)...))
    print(" $(length(samp)) points")
    # compute initial values and build surrogate
    samp_val = min_target.(sample) 
    current_min = scale(guess)
    current_val = surrogate(current_min)    
    min_index = length(sample)

    while length(sample) < f_calls
        @assert length(sample) == length(samp_val)
        # Inner loop: create surrogate and perform NM minimization on it.
        surrogate = RadialBasis(
            sample, samp_val, scale(lb), scale(ub), rad=cubicRadial();
        ...
        if any(new_min .< scale(lb)) || any(new_min .> scale(ub))
            ...
            sample = vcat(sample, new_samples)
        ...
        current_min = collect(sample[min_index])
    ...
    return (
        ...
        vmin=current_val, fcalls=length(sample)
    ), unscale.(sample), samp_val
end