...
function main()
    ...
    sur_res_e1 = surrogate_optimize(min_target, EI(), scale(lb), scale(ub), surrogate, SobolSample(), num_new_samples=10, maxiters=500)
    push!(
        benchs,
        (
    ...
    sur_res_g1e1 = surrogate_optimize(min_target, EI(), scale(lb), scale(ub), surrogate_g1e1, SobolSample(), num_new_samples=10, maxiters=1000)
    push!(
        benchs,
        (
